...
export class PaymentUploadStatementComponent implements OnInit {
  ...
  uploadListener($event) {
    ...
    this.formData.append("statementFile", this.upload.statementFile[0], this.upload.fileName);
  }
  ...
  uploadStatement() {
    ...
    this.uploadService.upload(this.formData).subscribe(
      ...
      err => {
        this.isLoading = false;
        this.snackBar.open(this.translations.UPLOAD_ERROR, this.viewContainerRef);
      });
  }
  ...
  confirmStatement() {
    this.confirmDialog.open(null, ok => {
      if (ok) {
        const body = 'bankName=' + this.uploadResponse.bankName +
          '?fileId=' + this.uploadResponse.fileId;

        this.isLoading = true;
        this.uploadService.confirm(body).subscribe(
          response => {
            this.snackBar.open(this.translations.CONFIRM_OK, this.viewContainerRef);
            this.showSummary = true;
            this.uploadResponse = {};
          },
          err => {
            this.snackBar.open(this.translations.CONFIRM_ERROR, this.viewContainerRef);
