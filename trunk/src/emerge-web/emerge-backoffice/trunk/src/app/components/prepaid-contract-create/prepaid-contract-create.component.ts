...
export class PrepaidContractCreateComponent implements OnInit {
  ...
  create() {
    ...
    this.isLoading = true;
    console.log('create from my system', this.prepaidContract);
  ...
  confirm(contract: ContractPrepaid) {
    this.confirmDialog.open(null, ok => {
      if (ok) {
        this.contractService.confirmPrepaidContract(this.customerId,
          contract.contractId)
          .subscribe(response => {
  ...
  close(serviceId: number, contractId: number, index: number) {
    this.confirmDialog.open(null, ok => {
      if (ok) {
        this.contractService.closePrepaidContract(
          this.customerId, contractId, serviceId).subscribe(response => {
